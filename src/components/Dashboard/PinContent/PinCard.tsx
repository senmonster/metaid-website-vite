...
import { IconDots, IconHelp, IconStar } from "@tabler/icons-react";
...
const PinCard = ({ p }: Iprops) => {
	...
	const popToolTip: React.ReactNode = (
		<div className="flex flex-col gap-3 text-wrap break-all">
			<div>
				There are a total of sixteen levels, and we use the number of stars to represent the
				current level, with more stars indicating a higher level and a rarer Pin.
				{/* This value represents the rarity level of the current pin, with higher levels being rarer. */}
				The following are the colors corresponding to different rarity levels:
			</div>
	...
	const popArr = pData.pop.split("");
	const firstNonZeroIndex = popArr.findIndex((v) => v !== "0"); // number of zeroes

	const level = firstNonZeroIndex < 22 ? 2 : firstNonZeroIndex - 21;
				...
				<div className="flex gap-2 items-center">
					...
					<>
						{level === -1
							? "--"
							: [...Array(8)].map(() => {
									return <IconStar className="w-2 h-2" />;
							  })}
					</>
