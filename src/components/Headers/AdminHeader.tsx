...
export default function AdminHeader({ burger }: Props) {
  ...
  const setBtcConnector = useSetRecoilState<IBtcConnector | null>(
    btcConnectorAtom
  );
    ...
    if (hasMetaid && !isNil(userInfo)) {
      return (
        ...
        >
          ...
          <Menu.Dropdown>
            <Menu.Item onClick={() => navigate('/dashboard/my-pin')}>
              My Pin
            </Menu.Item>
        ...
        <header className={cls(classes.header, 'pt-3 px-3')}>
          ...
          /> */}
            <Menu shadow='md' width={200}>
              <Menu.Target>
                <Button variant='light'>{network ?? 'testnet'}</Button>
              </Menu.Target>
